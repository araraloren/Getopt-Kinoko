...
=end item
=head3 method insert-normal(Str $opts) returns OptionSet

=begin head3

Insert a normal group into C<OptionSet>. Normal group is main group of the C<OptionSet>, it has many Option can set by user at the same time.

=end head3

=head3 method insert-multi(Str $opts) returns OptionSet
...
=head3 method insert-radio(Str $opts, :$force = False) returns OptionSet
